...
const fetchCaptcha = async () => {
    try {
        const response = await OpenHttpService.get('/open/getCaptcha', { scene: 'reg' });
        if (response.ret === 200) {
...
const checkCaptcha = async () => {
    try {
        const response = await OpenHttpService.post('/open/checkCaptcha', { scene: 'reg', value: verify_code.value });
...
const sendMessageCode = async () => {
    try {
        const response = await OpenHttpService.post('/open/sendCode', { scene: 'reg', user_phone: regForm.value.user_phone });
        if (response.ret === 200 && response.data) {
            toast.add({
                severity: 'success',
                summary: '成功',
...
async function doReg() {
    try {
        const data = await OpenHttpService.post('/open/reg', { ...regForm.value });
        if (data.ret === 200 && data.data) {
            message.value = '您已经注册成功，现在可以直接登录!';
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full surface-card py-8 px-5 sm:px-8" style="border-radius: 53px">
                    <div class="text-center mb-5">
                        <div class="text-900 text-3xl font-medium mb-3">用户注册</div>
                    </div>

                    <div>
                        <label for="email1" class="block text-900 text-xl font-medium mb-2">手机号</label>
                        <InputText id="email1" type="text" placeholder="请输入手机号" class="w-full md:w-30rem mb-5" style="padding: 1rem" v-model="regForm.user_phone" />
                        ...
                        <div class="captcha-container">
                            ...
                            <div class="captcha-row">
                                <InputText class="mr-2" v-model="regForm.phone_code" id="verify_code" placeholder="输入手机验证码" />
                        ...
                        <label for="password1" class="block text-900 font-medium text-xl mb-2">设置密码</label>
                        <Password id="password1" v-model="regForm.user_pass" placeholder="设置密码" :toggleMask="true" :feedback="false" class="w-full mb-3" inputClass="w-full" :inputStyle="{ padding: '1rem' }"></Password>

                        <div class="flex align-items-center justify-content-between mb-5 gap-5">
                            <div class="flex align-items-center">
                                <a class="font-medium no-underline ml-2 text-right cursor-pointer" href="/index/login" style="color: var(--primary-color)">已有账号？登录</a>
                            </div>
